class MapaModel {
    ...
    //TODO powinny byc dwie niezalezne mapy i klasy MapaBoolean, MapaChar, albo nawet Mapa<E>
...
MapaModel(){
    ...
    /*boolean poleJestPuste(int y,int x){
return true;
    }*/


    void inicjujMapePozycjiStatkow(){
        Logger.debug("inicjujMapePozycjiStatkow()");
    ...
    /*
        ...
        */
   private boolean poleNalezyDoObszaruPlanszy(int y,int x){
       if (y<0 | y>=PLANSZA_MAX_Y | x<0 | x>=PLANSZA_MAX_X) {
           Logger.deepDebug("Pole ("+x+","+y+") lezy poza obszarem planszy.");
           Logger.stackTrace();
           return false;
       }
       else
    ...
    boolean  moznaPostawicStatek(int y, int x){
        ...
        if (!poleNalezyDoObszaruPlanszy(y,x)) {
            Logger.debug("Proba postawienia statku poza obszarem planszy.");
            return false;
        }
        ...
        /* jesli sasiednie pole jest na planszy, to jesli jest na nim juz statek to zwieksz sumaTrue o 1;
         i tym samym zwroc false - nie mozna tu postawic statku*/
        for(int i=0;i<MacierzSasiedztwa.ILOSC_ELEMENTOW_MACIERZY_SASIEDZTWA;i++)
            if(poleNalezyDoObszaruPlanszy(y+ms.pola[i].getY(),x+ms.pola[i].getX()))
                if(mapaPozycjiStatkow[y+ms.pola[i].getY()][x+ms.pola[i].getX()]) sumaTrue++;
